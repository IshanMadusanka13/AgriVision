...
const TRAIN_AREA = 256; // model trained for 256m2
const MAX_PLANT_YIELD = 0.15; // 150g per plant
...
export default function ResultScreen() {
  ...
  // scale yield from 256m2 to actual field
  const totalYield =
    predictedYield && result?.area_m2
      ? predictedYield * (result.area_m2 / TRAIN_AREA)
      : null;

  // yield per plant
  const yieldPerPlant =
    totalYield && hasPlantCount
      ? Math.min(totalYield / result.total_plant_count, MAX_PLANT_YIELD)
      : null;
      ...
      >
        ...
        {yieldPrediction && (
          <View style={styles.yieldCard}>
            ...
            {hasPlantCount && yieldPerPlant && (
              <Text style={styles.perPlantText}>
                {result.total_plant_count} plants ×{" "}
                {yieldPerPlant.toFixed(3)} kg per plant
              </Text>
            )}
          </View>
        )}
